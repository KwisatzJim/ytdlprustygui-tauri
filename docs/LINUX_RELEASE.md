...
Status: the x86-64 AppImage has been built on Ubuntu and manually tested successfully. The Ubuntu-server build verified copyright notices for all 178 bundled ELF libraries with 0 unresolved libraries. Version 0.3.0 has been built and audited on the Ubuntu server and is prepared for GitHub publication. Future Linux candidates are built on the project's Ubuntu server.
